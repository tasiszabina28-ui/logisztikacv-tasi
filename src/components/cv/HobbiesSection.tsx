...
const HobbiesSection = () => {
  ...
  return (
    <section className="rounded-3xl shadow-sm p-8 my-8 transition-colors duration-300 bg-white border border-gray-100 dark:bg-zinc-900 dark:border-zinc-800">
      ...
      <div className="flex flex-wrap gap-4">
        {hobbies.map((hobby, index) => (
          <div key={index} className={`flex items-center gap-3 px-5 py-3 rounded-2xl border transition-all hover:scale-105 shadow-sm ${hobby.color || "bg-gray-50 text-gray-700 border-gray-200 dark:bg-zinc-800 dark:text-gray-300 dark:border-zinc-700"}`}>
            <hobby.icon size={20} strokeWidth={2} />
            <span className="font-semibold text-sm tracking-wide">{hobby.label}</span>
          </div>
        ))}
      </div>
    </section>
